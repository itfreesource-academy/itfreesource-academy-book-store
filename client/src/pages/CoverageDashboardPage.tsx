  ...
  return (
    <div className="space-y-8 pb-16" data-testid="coverage-dashboard-page">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {/* Lines */}
        <div className="bg-slate-900/90 rounded-2xl p-5 border border-slate-800 shadow-xl backdrop-blur-xl">
          <div className="flex justify-between items-start mb-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Line Coverage</span>
          ...
          <div className="text-2xl font-black text-white">{metrics.linesPct.toFixed(1)}%</div>
        ...
        <div className="bg-slate-900/90 rounded-2xl p-5 border border-slate-800 shadow-xl backdrop-blur-xl">
          ...
          <div className="text-2xl font-black text-white">{metrics.statementsPct.toFixed(1)}%</div>
          <div className="w-full bg-slate-800 rounded-full h-2 mt-3 overflow-hidden">
        ...
        <div className="bg-slate-900/90 rounded-2xl p-5 border border-slate-800 shadow-xl backdrop-blur-xl">
          <div className="flex justify-between items-start mb-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Functions</span>
          ...
          <div className="text-2xl font-black text-white">{metrics.functionsPct.toFixed(1)}%</div>
          <div className="w-full bg-slate-800 rounded-full h-2 mt-3 overflow-hidden">
        ...
        <div className="bg-slate-900/90 rounded-2xl p-5 border border-slate-800 shadow-xl backdrop-blur-xl">
          <div className="flex justify-between items-start mb-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Branches</span>
          ...
          <div className="text-2xl font-black text-white">{metrics.branchesPct.toFixed(1)}%</div>
          <div className="w-full bg-slate-800 rounded-full h-2 mt-3 overflow-hidden">
      ...
      <div className="bg-slate-900/90 rounded-2xl p-5 border border-slate-800 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4 backdrop-blur-xl">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-emerald-950/80 text-emerald-400 border border-emerald-800/60 flex items-center justify-center font-black text-lg">
            ✓
          </div>
          <div>
            <div className="text-sm font-extrabold text-white flex items-center gap-2">
              <span>All {metrics.totalTests} Unit & API Tests Passing</span>
              <span className="px-2 py-0.5 bg-emerald-950 text-emerald-300 border border-emerald-800 text-xs font-bold rounded-full">
                100% Pass Rate
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">
              4 Executed Test Suites: <code className="bg-slate-950 px-1 py-0.5 rounded text-indigo-300 border border-slate-800">borrow.test.ts</code>, <code className="bg-slate-950 px-1 py-0.5 rounded text-indigo-300 border border-slate-800">auth.test.ts</code>, <code className="bg-slate-950 px-1 py-0.5 rounded text-indigo-300 border border-slate-800">store.test.ts</code>, <code className="bg-slate-950 px-1 py-0.5 rounded text-indigo-300 border border-slate-800">api.test.ts</code>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-xs font-bold text-slate-300 hover:text-white px-3 py-1.5 rounded-lg border border-slate-700 hover:bg-slate-800 transition flex items-center gap-1.5"
          >
      ...
      <div className="border-b border-slate-800 flex gap-4 text-sm font-semibold">
        ...
        >
          ...
          <span className="px-1.5 py-0.2 bg-indigo-950 text-indigo-300 border border-indigo-800 rounded-full text-[10px] font-bold">
        ...
        <div className="space-y-6">
          <div className="bg-slate-900/90 rounded-3xl border border-slate-800 overflow-hidden shadow-2xl backdrop-blur-xl">
            <div className="p-4 sm:p-6 border-b border-slate-800 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-white text-base">Key Engine Modules & Coverage</h3>
                <p className="text-xs text-slate-400 mt-0.5">
                ...
                <tbody className="divide-y divide-slate-800/80">
                  {modules.map((m, idx) => (
                    <tr key={idx} className="hover:bg-slate-800/50 transition">
                      ...
                      <td className="py-4 px-4 font-mono text-slate-300 text-xs">{m.path}</td>
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-white w-10">{m.lines.toFixed(1)}%</span>
                          <div className="w-24 bg-slate-800 rounded-full h-1.5 overflow-hidden">
                            <div className={`h-1.5 rounded-full ${getBarColor(m.lines)}`} style={{ width: `${m.lines}%` }} />
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-4 font-medium text-slate-200">{m.functions.toFixed(1)}%</td>
                      <td className="py-4 px-4 font-medium text-slate-200">{m.branches.toFixed(1)}%</td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-slate-900/90 p-5 rounded-2xl border border-slate-800 shadow-xl space-y-2 backdrop-blur-xl">
              <div className="flex justify-between items-center">
                <span className="font-bold text-white text-sm">⏱️ borrow.test.ts (10 Tests)</span>
                <span className="text-xs text-emerald-300 bg-emerald-950 border border-emerald-800 px-2 py-0.5 rounded-full font-bold">Passed</span>
              </div>
            ...
            <div className="bg-slate-900/90 p-5 rounded-2xl border border-slate-800 shadow-xl space-y-2 backdrop-blur-xl">
              <div className="flex justify-between items-center">
                <span className="font-bold text-white text-sm">👥 auth.test.ts (10 Tests)</span>
                <span className="text-xs text-emerald-300 bg-emerald-950 border border-emerald-800 px-2 py-0.5 rounded-full font-bold">Passed</span>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
            ...
            <div className="bg-slate-900/90 p-5 rounded-2xl border border-slate-800 shadow-xl space-y-2 backdrop-blur-xl">
              <div className="flex justify-between items-center">
                <span className="font-bold text-white text-sm">📦 store.test.ts (8 Tests)</span>
                <span className="text-xs text-emerald-300 bg-emerald-950 border border-emerald-800 px-2 py-0.5 rounded-full font-bold">Passed</span>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
            ...
            <div className="bg-slate-900/90 p-5 rounded-2xl border border-slate-800 shadow-xl space-y-2 backdrop-blur-xl">
              <div className="flex justify-between items-center">
                <span className="font-bold text-white text-sm">🌐 api.test.ts (12 Tests)</span>
                <span className="text-xs text-emerald-300 bg-emerald-950 border border-emerald-800 px-2 py-0.5 rounded-full font-bold">Passed</span>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
      ...
      {activeTab === 'external' && (
        <div className="space-y-6">
          <div className="bg-slate-900/90 rounded-2xl border border-slate-800 p-6 shadow-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 backdrop-blur-xl">
            <div>
              <h3 className="text-lg font-bold text-white">Separate Automation Suite Reports</h3>
            ...
            <button
              ...
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-indigo-600/30 transition flex items-center gap-1.5 self-start sm:self-auto"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {externalReports.map((report) => (
              <div key={report.id} className="bg-slate-900/90 rounded-2xl border border-slate-800 p-5 shadow-xl space-y-4 backdrop-blur-xl">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-indigo-950 border border-indigo-800/60 text-indigo-300 flex items-center justify-center font-bold text-sm uppercase">
                      {report.tool.slice(0, 2)}
                    </div>
                    <div>
                      <h4 className="font-bold text-white text-sm">{report.name}</h4>
                ...
                <div className="grid grid-cols-3 gap-2 bg-slate-950 p-3 rounded-xl text-center border border-slate-800">
                  <div>
                    <span className="text-[10px] text-slate-400 block uppercase font-semibold">Total Tests</span>
                    <span className="text-base font-black text-white">{report.totalTests}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block uppercase font-semibold">Passed</span>
                    <span className="text-base font-black text-emerald-400">{report.passedTests}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 block uppercase font-semibold">Failed</span>
                    <span className={`text-base font-black ${report.failedTests > 0 ? 'text-rose-400' : 'text-slate-500'}`}>
                      {report.failedTests}
                    </span>
                  </div>
                </div>
      ...
      {activeTab === 'cloud' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900/90 rounded-2xl border border-slate-800 p-6 shadow-xl space-y-4 backdrop-blur-xl">
            <div className="w-10 h-10 rounded-xl bg-slate-950 text-slate-300 border border-slate-800 flex items-center justify-center font-bold text-lg">
              🐙
            </div>
            <h3 className="font-bold text-white text-base">GitHub Actions Step Summary</h3>
            <p className="text-xs text-slate-400 leading-relaxed">
          ...
          {/* Codecov Integration */}
          <div className="bg-slate-900/90 rounded-2xl border border-slate-800 p-6 shadow-xl space-y-4 backdrop-blur-xl">
            <div className="w-10 h-10 rounded-xl bg-rose-950 text-rose-400 border border-rose-800/60 flex items-center justify-center font-bold text-lg">
              🎯
            </div>
            <h3 className="font-bold text-white text-base">Codecov.io Free Dashboard</h3>
            <p className="text-xs text-slate-400 leading-relaxed">
          ...
          <div className="bg-slate-900/90 rounded-2xl border border-slate-800 p-6 shadow-xl space-y-4 backdrop-blur-xl">
            <div className="w-10 h-10 rounded-xl bg-blue-950 text-blue-400 border border-blue-800/60 flex items-center justify-center font-bold text-lg">
              📄
            </div>
            <h3 className="font-bold text-white text-base">Interactive HTML Line Inspector</h3>
            <p className="text-xs text-slate-400 leading-relaxed">
      ...
      {isImportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 overflow-y-auto">
          <div className="bg-slate-900 rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-800 animate-in fade-in zoom-in duration-150">
            <div className="bg-gradient-to-r from-slate-950 via-indigo-950 to-slate-950 p-6 text-white border-b border-slate-800 flex items-center justify-between">
              <div>
              ...
              <button
                ...
                className="text-slate-400 hover:text-white rounded-full w-8 h-8 flex items-center justify-center hover:bg-slate-800"
              >
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleImportExternal} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Suite / Repo Name</label>
                <input
                  type="text"
                  value={importName}
                  onChange={(e) => setImportName(e.target.value)}
                  placeholder="e.g. Cypress Regression Suite"
                  className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">Automation Tool</label>
                  <select
                    value={importTool}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">Code Coverage %</label>
                  <input
                    type="number"
                    step="0.1"
                    min="0"
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">Total</label>
                  <input
                    type="number"
                    min="1"
                    value={importTotal}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">Passed</label>
                  <input
                    type="number"
                    min="0"
                    value={importPassed}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">Failed</label>
                  <input
                    type="number"
                    min="0"
                    value={importFailed}
