    ...
    <div className="space-y-6 pb-16" data-testid="audit-logs-page">
      ...
      <div className="bg-slate-900/90 rounded-2xl border border-slate-800 p-6 shadow-2xl backdrop-blur-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-extrabold text-white" data-testid="audit-title">
              System Audit Trail
            </h1>
        ...
        {/* Search */}
        <div className="flex items-center gap-2 px-3 py-2 border border-slate-700 rounded-xl bg-slate-950 max-w-xs w-full">
          ...
          <input
            ...
            className="w-full text-xs bg-transparent outline-none text-slate-100 placeholder-slate-500"
          />
        </div>
      </div>
      ...
      <div className="bg-slate-900/90 rounded-2xl border border-slate-800 overflow-hidden shadow-2xl backdrop-blur-xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-300" data-testid="audit-table">
            ...
            <tbody className="divide-y divide-slate-800" data-testid="audit-table-body">
              {loading ? (
                <tr>
                ...
                <tr>
                  <td colSpan={6} className="p-8 text-center text-slate-500">No matching audit logs recorded.</td>
                </tr>
              ) : (
                filteredLogs.map((log) => (
                  <tr key={log.id} data-testid={`audit-row-${log.id}`} className="hover:bg-slate-800/60 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <span
                        ...
                        className="px-2 py-0.5 rounded font-mono font-bold text-[10px] bg-indigo-950/60 text-indigo-300 border border-indigo-800/80"
                      >
                        {log.action}
                      </span>
                    </td>
                    ...
                    <td className="py-3 px-4 max-w-md text-slate-300 truncate" title={log.details}>
                      {log.details}
                    </td>
