...
| 2026-09-17 | *(current)* | **Enterprise Storefront Polish & Universal Light/Dark Theme (Web + Android):** <br>1. **Web**: Replaced the "QA Engineering Showcase" banner on `HomePage.tsx` with an authentic enterprise reading banner and real reader metrics. Completely overhauled `AboutPage.tsx` with high-concurrency edge architecture, creator attribution to Vishal Prajapati, and a dedicated CTA routing candidates to the Practical Quality Engineering Course on `academy.itfreesource.com` (clearing out internal persona passwords). Redesigned `Footer.tsx` and `Navbar.tsx` for enterprise polish. Converted `Modal.tsx`, `DatePicker.tsx`, `SearchableSelect.tsx`, `MultiSelect.tsx`, `BookDetailPage.tsx`, `BorrowedBooksPage.tsx`, and `UsersPage.tsx` to fully adaptive light/dark mode.<br>2. **Android**: Cleaned `NavigationComponent.kt` by removing "QA Lab" from consumer navigation (`Catalog`, `Cart`, `Orders`, `Rentals`), making bottom bar and `AttributionFooter.kt` theme-aware. Converted `BookCard.kt`, `AddToCartControl.kt`, `CatalogScreen.kt`, `BookDetailDialog.kt`, `CartCheckoutScreen.kt`, `OrdersScreen.kt`, and `RentalsScreen.kt` to adaptively render crisp white/light cards in light mode. Added course banner to `ProfileDialog.kt`. Rebuilt signed release APK (`6.0MB`). | User: "I want the candidate to have a real feel of enterprise application and nothing like this, anywhere except about section where we are deliberately very few things and asking candidate to go to our practical engineering course on academy... same with android ap" |
